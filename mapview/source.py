...
from math import cos, ceil, log, tan, pi, atan, exp, floor
...
class MapSource(object):
    ...
    def fill_tile(self, tile):
        ...
        if self.bounds is not None and self.restrict_to_bounds:
            min_x, min_y, max_x, max_y = self.get_bounds_xy(self.bounds, tile.zoom)
            if (tile.tile_x < min_x or tile.tile_x > max_x or
                tile.tile_y < min_y or tile.tile_y > max_y):
                tile.state = "done"
                return

        Downloader.instance(cache_dir=self.cache_dir).download_tile(tile)

    def get_bounds_xy(self, bounds, zoom):
        min_lon, min_lat, max_lon, max_lat = bounds
        tile_size = self.dp_tile_size
        min_x = floor(self.get_x(zoom, min_lon) / tile_size)
        max_x = ceil(self.get_x(zoom, max_lon) / tile_size)
        min_y = floor(self.get_y(zoom, min_lat) / tile_size)
        max_y = ceil(self.get_y(zoom, max_lat) / tile_size)
